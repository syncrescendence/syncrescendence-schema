...
The tier model decomposes the institution by function, not by surface, team, or vendor. Each tier carries a single governing purpose. Tiers interact through declared interfaces: what one tier provides, the adjacent tier consumes. No tier reaches into another tier's internals — interaction is always mediated by the interface contract.
...
Seven tiers and three orthogonal concerns. What follows specifies each at one resolution level below the root schema.
